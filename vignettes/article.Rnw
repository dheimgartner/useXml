...
formatcom=\color{magenta},
fontshape=sl
}

\newcommand{\xml}[1]{%
{\color{magenta}\textsl{\Verb|#1|}}%
}
...
Since the above list contains key duplicates (i.e., multiple person elements) it is ``difficult'' to access elements via keys and one might be tempted to use absolute indices... Let's use \XPath instead!
<<>>=
## absolute
adrian <- xml2::xml_find_all(xml_parsed, xpath = "/people/person[last()]")
## does not depend on order...
adrian <- xml2::xml_find_all(xml_parsed, xpath = "/people/person[name = 'Adrian']")
@
